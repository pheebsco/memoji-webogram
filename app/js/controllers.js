...
.controller('AppLoginController', function ($scope, $rootScope, $location, $timeout, $modal, $modalStack, MtpApiManager, ErrorService, NotificationsManager, PasswordManager, ChangelogNotifyService, IdleManager, TelegramMeWebService, _) {
  ...
  $scope.sendCode = function () {
    ...
    function normalizePhoneNumber(number) {
      function removeFromFirst(str, p) {
        if (str.indexOf(p) == 0) {
      ...
      number = removeFromFirst(number, '+');
      number = removeFromFirst(number, '0');
      number = removeFromFirst(number, '0');
      number = removeFromFirst(number, '98');
      number = removeFromFirst(number, '0');
      return '+98' + number;
    }

    var normalizedPhone = normalizePhoneNumber($scope.credentials.phone_number || '');
    $scope.credentials.phone_full = normalizedPhone;
    var fullPhone = normalizedPhone;
