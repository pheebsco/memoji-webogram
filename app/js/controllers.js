...
function getQueryParams(window) {
...
function downloadFile(url, cb) {
  ...
  console.log("GONNA DL:", url);
  request.open("GET", url);
  request.responseType = "blob";
  request.addEventListener("progress", function (e) {
    var percentComplete = e.lengthComputable ? (e.loaded * 70 / e.total) | 0 : 0;
  ...
  request.addEventListener("load", function (e) {
    console.log("DL SUCCESSFUL:", URL.createObjectURL(this.response));
...
angular.module('myApp.controllers', ['myApp.i18n'])
  .controller('PackageController', function ($q, $scope, $location, $http, $window, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, MtpApiManager) {
    ...
    function loadImage(uri, cb) {
      ...
      image.src = URL.createObjectURL(uri);
    }
      ...
      async.waterfall([
        ...
        function (layout, cb) {
          ...
          async.parallel({
            ...
            if (err) {
              console.log("LOADING IMAGES ERR", err);
              return cb(err);
            }
    ...
    function downloadHead(purchase, cb) {
      downloadFile(purchase.head.resource, function (err, head) {
        if (err) {
          return cb(err);
        }
        purchase.head = head;
    ...
    function initiatePackage(purchase, cb) {
      AppMessagesManager.sendText(429000, "/cancel", {});
      AppMessagesManager.sendText(429000, "/newpack", {});
      AppMessagesManager.sendText(429000, purchase.package.name, {});
      cb(null, purchase);
    }
    ...
    function sendEmoji(emoji, cb) {
      AppMessagesManager.sendText(429000, emoji, {}, cb);
    }

    function processSticker(head, sticker, cb) {
      async.waterfall([
        async.apply(generateSticker, sticker, head),
        uploadImage,
        async.apply(sendEmoji, sticker.emoji)
      ], cb);
    }

    function processStickers(purchase, cb) {
      console.log("STICKERS TO BE PROCESSED", purchase.package.stickers)
      // purchase.package.stickers = [purchase.package.stickers[0]];
      async.mapSeries(purchase.package.stickers, async.apply(processSticker, purchase.head),
        function (err) {
          cb(err, purchase);
        });
    }

    function checkStickersInPack(purchase, cb){
      AppMessagesManager.expectMessage("Stickers in the pack: "+ purchase.package.stickers.length, true, function(err, msg){
          console.log("STICKER FULLY IN PACK:", msg.message);
          return cb(err);
      })
    }
    ...
    function checkKaboom(packID, cb) {
      AppMessagesManager.expectMessage(packID, true, function(err, message){
        if(err){
          return cb(err);
        }
        cb(null, packID);
...
.controller('AppLoginController', function ($scope, $rootScope, $location, $timeout, $modal, $modalStack, MtpApiManager, ErrorService, NotificationsManager, PasswordManager, ChangelogNotifyService, IdleManager, TelegramMeWebService, _) {
  ...
  function selectPhoneCountryByIso2(countryIso2) {
    ...
    return selectCountry({
      name: _('country_select_modal_country_us_raw'),
      code: '+1'
    })
  }
...
.controller('AppIMController', function ($q, qSync, $scope, $location, $routeParams, $modal, $rootScope, $modalStack, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ContactsSelectService, ChangelogNotifyService, ErrorService, AppRuntimeManager, HttpsMigrateService, LocationParamsService, AppStickersManager) {
  ...
  function getDialogs(force) {
    ...
    if (searchMessages) {
      ...
      return searchTimeoutPromise.then(function () {
        ...
        return AppMessagesManager.getSearch(searchPeerID, $scope.search.query, {
...
.controller('AppImHistoryController', function ($scope, $location, $timeout, $modal, $rootScope, toaster, _, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, PeersSelectService, IdleManager, StatusManager, NotificationsManager, ErrorService, GeoLocationManager) {
  ...
  function showMoreHistory() {
    ...
    var inputMediaFilter = $scope.historyFilter.mediaType && {
...
.controller('AppImSendController', function ($rootScope, $q, $scope, $timeout, MtpApiManager, Storage, AppProfileManager, AppChatsManager, AppUsersManager, AppPeersManager, AppDocsManager, AppMessagesManager, AppInlineBotsManager, MtpApiFileManager, DraftsManager, RichTextProcessor) {
  ...
  function updateMentions() {
    ...
    $q.all({
      pop: inlineBotsPromise,
      chat: chatParticipantsPromise
    }).then(function (result) {
      var done = {}
...
.controller('AppFooterController', function ($scope) {
...
.controller('UserModalController', function ($scope, $location, $rootScope, $modalInstance, AppProfileManager, $modal, AppUsersManager, MtpApiManager, NotificationsManager, AppPhotosManager, AppMessagesManager, AppPeersManager, PeersSelectService, ErrorService) {
  ...
  $scope.flushHistory = function (justClear) {
    ErrorService.confirm({
      type: justClear ? 'HISTORY_FLUSH' : 'HISTORY_FLUSH_AND_DELETE'
    }).then(function () {
...
.controller('ChannelModalController', function ($scope, $timeout, $rootScope, $modal, AppUsersManager, AppChatsManager, AppProfileManager, AppPhotosManager, MtpApiManager, MtpApiFileManager, NotificationsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, ContactsSelectService, ErrorService) {
  ...
  $scope.leaveChannel = function () {
    return ErrorService.confirm({
      type: $scope.isMegagroup ? 'MEGAGROUP_LEAVE' : 'CHANNEL_LEAVE'
    }).then(function () {
  ...
  $scope.deleteChannel = function () {
    return ErrorService.confirm({
      type: $scope.isMegagroup ? 'MEGAGROUP_DELETE' : 'CHANNEL_DELETE'
    }).then(function () {
...
.controller('SettingsModalController', function ($rootScope, $scope, $timeout, $modal, AppUsersManager, AppChatsManager, AppPhotosManager, MtpApiManager, Storage, NotificationsManager, MtpApiFileManager, PasswordManager, ApiUpdatesManager, ChangelogNotifyService, WebPushApiManager, AppRuntimeManager, ErrorService, _) {
  $scope.profile = {}
  $scope.photo = {}
...
.controller('SessionsListModalController', function ($scope, $q, $timeout, _, MtpApiManager, ErrorService, $modalInstance) {
  ...
  $scope.terminateSession = function (hash) {
    ErrorService.confirm({
      ...
      MtpApiManager.invokeApi('account.resetAuthorization', {
        hash: hash
      }).then(updateSessions)
    })
  }
...
.controller('ChatInviteLinkModalController', function (_, $scope, $timeout, $modalInstance, AppChatsManager, AppProfileManager, ErrorService) {
  $scope.exportedInvite = {
    link: _('group_invite_link_loading_raw')
  }
  ...
  function updateLink(force) {
    ...
    if (chat.username) {
      $scope.exportedInvite = {
        link: 'https://telegram.me/' + chat.username,
        short: true
      }
      selectLink()
      return
...
.controller('StickersetModalController', function ($scope, $rootScope, $modalInstance, MtpApiManager, RichTextProcessor, AppStickersManager, AppDocsManager, AppMessagesManager, LocationParamsService) {
  ...
  AppStickersManager.getStickerset($scope.inputStickerset).then(function (result) {
    ...
    angular.forEach($scope.documents, function (doc) {
      ...
      $scope.stickerDimensions[doc.id] = {
        width: dim.w,
        height: dim.h
      }
    })
